...
-- ========================================
-- 8. Star-Schema Views (Enriched & Per-Crop)
-- ========================================

-- Base enriched view joining fact and dimensions
CREATE OR REPLACE VIEW climatecrop.vw_crop_yield_enriched AS
SELECT
    f.fact_id,
    c.crop_id,
    c.crop_name,
    ...
    t.season,
    f.area_acres,
    f.avg_yield_maunds_per_acre,
    f.avg_yield_kg_per_acre,
    f.production_kg,
    f.production_tons,
    f.avg_price_per_kg,
    f.total_revenue_pkr,
...
-- Crop-specific views (one per crop)
CREATE OR REPLACE VIEW climatecrop.vw_cotton AS
SELECT *
FROM climatecrop.vw_crop_yield_enriched
WHERE crop_name = 'Cotton';

CREATE OR REPLACE VIEW climatecrop.vw_maize AS
SELECT *
FROM climatecrop.vw_crop_yield_enriched
WHERE crop_name = 'Maize';

CREATE OR REPLACE VIEW climatecrop.vw_rice AS
SELECT *
FROM climatecrop.vw_crop_yield_enriched
